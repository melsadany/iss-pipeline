...
extract_attention_features <- function(checkbox, hi, participant_id) {
  if (is.null(checkbox) || nrow(checkbox) == 0 || is.null(hi) || nrow(hi) == 0) {
    return(tibble::tibble(
      participant_id = participant_id,
      checkbox_hit_rate = NA_real_, checkbox_mean_rt = NA_real_, checkbox_rt_sd = NA_real_,
      hi_hit_rate = NA_real_, hi_mean_rt = NA_real_, hi_sd_rt = NA_real_
    ))
  }
...
extract_memory_features <- function(wmemory, participant_id) {
  if (is.null(wmemory) || nrow(wmemory) == 0) {
    return(tibble::tibble(
      participant_id = participant_id,
      recall_time = NA_real_, exact_match = NA_real_, levenshtein_dist = NA_real_,
      phonological_similarity = NA_real_, recall_score = NA_real_
    ))
  }
...
extract_sent_rep_features <- function(sent_rep, participant_id) {
  if (is.null(sent_rep) || nrow(sent_rep) == 0) {
    return(tibble::tibble(
      participant_id = participant_id,
      response_duration = NA_real_, levenshtein_dist = NA_real_, word_accuracy = NA_real_,
      bigram_accuracy = NA_real_, word_count_diff = NA_real_, lcs_length = NA_real_,
      word_order_score = NA_real_
    ))
  }
...
extract_reading_features <- function(reading, participant_id) {
  if (is.null(reading) || nrow(reading) == 0) {
    return(tibble::tibble(
      participant_id = participant_id,
      valence = NA_character_, reading_latency = NA_real_,
      is_correct = NA_real_, substitution = NA_real_
    ))
  }
